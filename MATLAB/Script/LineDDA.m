...
if not(libisloaded('Gdi32'))
    loadlibrary('Gdi32.dll', @Gdi32proto);
end
...
%Execute
calllib('Gdi32', 'LineDDA', 0, 0, 1, 1, hAlloc, []);
...
function [fcns,structs,enuminfo] = Gdi32proto
    fcns=[]; structs=[]; enuminfo=[]; fcns.alias={};
    
    % Prototype for LineDDA
    fcns.name{1} = 'LineDDA';
    fcns.calltype{1} = 'cdecl';
    fcns.LHS{1} = 'int';
    fcns.RHS{1} = {'int', 'int', 'int', 'int', 'int64', 'voidPtr'};

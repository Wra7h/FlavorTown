...
%Execute
calllib('user32', 'EnumDisplayMonitors', [], [], hAlloc, []);
...
function [fcns, structs, enuminfo] = user32proto
    ...
    % Prototype for EnumDisplayMonitors
    fcns.name{1} = 'EnumDisplayMonitors';
    fcns.calltype{1} = 'cdecl';
    fcns.LHS{1} = 'int'; 
    fcns.RHS{1} = {'voidPtr', 'voidPtr', 'int64Ptr', 'voidPtr'};
end
...
function [fcns,structs,enuminfo] = msvcrtproto
    ...
    fcns.calltype{1} = 'cdecl';
    fcns.LHS{1} = 'voidPtr'; 
    fcns.RHS{1} = {'int64Ptr', 'uint8Ptr', 'int64'};
end